...
public class Ex1_GUI {
	public static void main(String[] a) {
        ...
        int samples = 16;
		assignments.Ex1.StdDraw.setScale(min, max);
		assignments.Ex1.StdDraw.clear();
	...
	public static void testGUI(double[] po1, double[] po2, double min, double max, int samples) {
		...
		assignments.Ex1.StdDraw.setPenColor(assignments.Ex1.StdDraw.GRAY);
		drawGrid(min,max);
        assignments.Ex1.StdDraw.setPenRadius(0.005);
		assignments.Ex1.StdDraw.setPenColor(assignments.Ex1.StdDraw.BLACK);
		drawArea(po1,po2, xx[0], 10, samples);
		assignments.Ex1.StdDraw.setPenColor(assignments.Ex1.StdDraw.BLUE);
		drawPoly(po1, min, max,n);
        drawInfo(po2,0,8);
		assignments.Ex1.StdDraw.setPenColor(assignments.Ex1.StdDraw.GREEN);
		drawPoly(po2, min, max,n);
		drawInfo(po1,0,7);

        assignments.Ex1.StdDraw.setPenColor(assignments.Ex1.StdDraw.BLACK);
		double area = Ex1.area(po1, po2, xx[0], 6, samples);
        assignments.Ex1.StdDraw.text(0,-8,"xx: "+ps);
		assignments.Ex1.StdDraw.text(0,-9,"Area:  "+area+",  "+samples+" samples");
	}
	public static void drawPoly(double[] poly, double x1, double x2, int numberOfElements) {
		...
		for(double x = x1+delta;x<=x2;x+=delta) {
			...
			assignments.Ex1.StdDraw.line(x0, y0, x, y1);
			x0=x;
			y0 = y1;
		} 
	} 
	public static void drawArea(double[] p1, double[] p2, double x1, double x2, int numberOfElements) {
		...
		for(double x = x1+delta;x<=x2;x+=delta) {
			...
			assignments.Ex1.StdDraw.setPenColor(assignments.Ex1.StdDraw.ORANGE);
			assignments.Ex1.StdDraw.filledPolygon(xx, yy);
			assignments.Ex1.StdDraw.setPenColor(assignments.Ex1.StdDraw.BLACK);
			assignments.Ex1.StdDraw.polygon(xx, yy);
			x0=x;
			y01 = y11;
			y02=y12;
		} 
	...
	public static void drawInfo(double[] poly, double x, double y) {
		...
		assignments.Ex1.StdDraw.text(x,y,s);
	}
	public static void drawGrid(double min, double max) {
		assignments.Ex1.StdDraw.line(0,min,0,max);
		assignments.Ex1.StdDraw.line(min,0,max,0);
	}
}
